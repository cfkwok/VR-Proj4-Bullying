...
public class SOneMovementA : MonoBehaviour {
    ...
    private SceneFadeInOutA sceneFader;
    ...
    void Start () {
        ...
        sceneFader = GameObject.Find("Fader").GetComponent<SceneFadeInOutA>();
    ...
    void Update()
    {
        ...
        if (friendRotate)
        {
            ...
            if (friendObject.transform.eulerAngles.y <= 220)
            {
                ...
                StartCoroutine(ChangeCaptionText(0.2f, captionText));
                captionText = "Friend: Remember what he did?!";
        ...
        */
        ...
        if (contBullyCount > 3 && bullyAnimator.GetCurrentAnimatorStateInfo(0).IsName("zombie_headbutt"))
        {
            if (bullyAnimator.GetCurrentAnimatorStateInfo(0).normalizedTime > 1)
            {
                ...
                bullyAudio.Play();
                StartCoroutine(StopBullyAudio(16f));
                captionText = "Bully: Come on, that all you got?";
                StartCoroutine(ChangeCaptionText(1f, captionText));
                captionText = "Victim: Stop...";
                StartCoroutine(ChangeCaptionText(4f, captionText));
                captionText = "Bully: That was a good beating. See you next week, sucker!";
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (!destReached)
        {
            ...
            StartCoroutine(StartBullyAudio(6.3f));
            StartCoroutine(StopBullyAudio(8.4f));
            friendAudio.Play();
            StartCoroutine(StopFriendAudio(5f));
            captionText = "Friend: Poor kid, it was only a week ago when he got beat up.";
            StartCoroutine(ChangeCaptionText(0.2f, captionText));
            captionText = ("Victim: ...");
            StartCoroutine(ChangeCaptionText(5.2f, captionText));
            captionText = ("Bully: You ready for more?!");
            StartCoroutine(ChangeCaptionText(6.5f, captionText));
            captionText = "";
            StartCoroutine(ChangeCaptionText(8.5f, captionText));

            StartCoroutine(BullyPunch(8.3f));            
            StartCoroutine(AllowDialogFriend(8.6f));
        }
    ...
    IEnumerator StartBullyAudio(float time)
    {
        yield return new WaitForSeconds(time);
        bullyAudio.Play();
    }
